...
class MelisEcomOrderTable extends MelisEcomGenericTable
{
    ...
    public function getOrderList(
        ...
        if (!is_null($orderStatusId) && is_null($onlyValid)) {
            $select->where->equalTo('melis_ecom_order.ord_status', (int)$orderStatusId);
        }
        ...
        if (!is_null($couponId)) {
            $select->where->equalTo('melis_ecom_coupon_order.cord_coupon_id', (int)$couponId);
        }

        if (!is_null($clientId)) {
            $select->where->equalTo('melis_ecom_order.ord_client_id', (int)$clientId);
        }

        if (!is_null($clientPersonId)) {
            $select->where->equalTo('melis_ecom_order.ord_client_person_id', (int)$clientPersonId);
        }
    ...
    public function getOrderStatusByOrderId($orderId, $langId = null)
    {
        ...
        $select->join(
            'melis_ecom_order_status',
            'melis_ecom_order_status.osta_id=melis_ecom_order.ord_status',
            array('*'),
            $select::JOIN_LEFT
        );
        $select->join(
        ...
        $select->where->equalTo('ord_id', (int)$orderId);
    ...
    public function getClientLastOrderByClientId($clientId)
    {
        ...
        $select->where->equalTo('ord_client_id', (int)$clientId);
    ...
    public function getCurrentMonth($onlyValid = null)
    {
        ...
        if (!empty($onlyValid)) {
            $select->where->notEqualTo('ord_status', -1);
        }
    ...
    public function getClientOrderDetailsById($orderId, $clientId, $personId = null, $langId = null)
    {
        ...
        if (!is_null($personId)) {
            $select->where->equalTo('ord_client_person_id', (int)$personId);
        }
    ...
    public function getOrderCouponByOrderId($orderId)
    {
        ...
        $select->join(
            'melis_ecom_coupon_order',
            'melis_ecom_coupon_order.cord_order_id=melis_ecom_order.ord_id',
            array('*'),
            $select::JOIN_LEFT
        );
