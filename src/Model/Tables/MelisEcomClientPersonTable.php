...
class MelisEcomClientPersonTable extends MelisEcomGenericTable
{
    ...
    public function getContactLists($accountId = null, $type = null, $status = null, $searchValue = '', $searchKeys = [], $start = null, $limit = null, $orderColumn = 'cper_id', $order = 'DESC', $defaultAccountOnly = false, $hasDefaultOnly = false, $count = false)
    {
        ...
        if (!empty($accountId))
            $select->where->equalTo('client.cli_id', (int)$accountId);

        if (!empty($type))
            $select->where->equalTo('cper_type', $type);

        if (!is_null($status) && $status != '')
            $select->where->equalTo('cper_status', (int)$status);
    ...
    public function getContactAssocAccountLists($contactId, $searchValue = '', $searchKeys = [], $start = null, $limit = null, $orderColumn = 'cli_id', $order = 'DESC', $count = false)
    {
        ...
        $select->join(['client' => 'melis_ecom_client'], 'client.cli_id = melis_ecom_client_person_rel.cpr_client_id', array('cli_id', 'cli_name', 'cli_status', 'cli_group_id'), $select::JOIN_LEFT);
        ...
        if (!empty($contactId))
            $select->where->equalTo('melis_ecom_client_person_rel.cpr_client_person_id', (int)$contactId);
    ...
    public function getClientPersonByClientId($clientId)
    {
        ...
        $select->join(
            'melis_ecom_client_person_rel',
            'melis_ecom_client_person_rel.cpr_client_person_id=melis_ecom_client_person.cper_id',
            array('*'),
            $select::JOIN_LEFT
        );
        $select->join(
        ...
        $select->where->equalTo('melis_ecom_client_person_rel.cpr_client_id', (int)$clientId);
    ...
    public function getClientPersonByPersonId($personId)
    {
        ...
        $select->join(
            ['client' => 'melis_ecom_client'],
            'client.cli_id = melis_ecom_client_person_rel.cpr_client_id',
            array('cli_id', 'cli_name', 'cli_status'),
            $select::JOIN_LEFT
        );
        $select->join(
            'melis_ecom_civility',
        ...
        if ($personId) {
            $select->where->equalTo('melis_ecom_client_person_rel.cpr_client_person_id', (int)$personId);
        }
    ...
    public function getClientPersonByClientIdPersonIdAndPersonEmail($clientId, $personId = null, $personEmail = null)
    {
        ...
        if (!empty($personEmail))
            $select->where->equalTo('melis_ecom_client_person.cper_email', (int)$personEmail);
    ...
    public function getContacts(array $options, $fixedCriteria = null)
    {
        ...
        $select->join(
            'melis_ecom_client',
            'melis_ecom_client.cli_id=melis_ecom_client_person.cper_client_id',
            array('*'),
            $select::JOIN_LEFT
        );
        ...
        if (!empty($options['client_id'])) {
            $select->where->equalTo('cper_client_id', (int)$options['client_id']);
        }
    ...
    public function checkEmailExist($email, $personId)
    {
        ...
        $select->where->equalTo('cper_email', $email);
        $select->where->notEqualTo('cper_id', (int)$personId);
        $resultData = $this->getTableGateway()->selectWith($select);
    ...
    public function getContactDefaultAccount($contactId)
    {
        ...
        $select->join(
            'melis_ecom_client',
            'melis_ecom_client_person_rel.cpr_client_id=melis_ecom_client.cli_id',
            array('*'),
            $select::JOIN_LEFT
        );

        $select->where(array('cper_id' => (int)$contactId));
