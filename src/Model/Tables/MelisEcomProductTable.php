...
class MelisEcomProductTable extends MelisEcomGenericTable
{
    ...
    public function getProduct(
        ...
        if (!is_null($onlyValid))
            $clause['melis_ecom_product.prd_status'] = (int)$onlyValid;
    ...
    public function getProductList($categoryIds = array(), $countryId = null, $onlyValid = null,  $start = null, $limit = null, $orderColumn = 'prd_id', $order = 'ASC', $search = '')
    {
        ...
        if (!empty($countryId)) {
            $select->where->NEST->equalTo('melis_ecom_price.price_country_id', (int)$countryId)
                ->or->equalTo('var_price.price_country_id', (int)$countryId);
        }
    ...
    public function getProductByName($productName, $langId = null)
    {
        ...
        $select->where->NEST->like('melis_ecom_product.prd_reference', $productName)
            ...
            ->and->equalTo('product_text.ptxt_lang_id', (int)$langId)
    ...
    public function getProductByTextAndType($productText, $productType = array('TITLE'), $categoryIds = array(), $langId = null, $status = 1, $start = 0, $limit = null)
    {
        ...
        if (!is_null($langId)) {
            $select->where->and->equalTo('melis_ecom_product_text.ptxt_lang_id', (int)$langId);
        }
    ...
    public function getProductByAttributeValueIdsAndPriceRange($attrValIds = array(), $minPrice = null, $maxPrice = null, $categoryIds = array(), $countryId = null, $status = 1, $start = 0, $limit = null)
    {
        ...
        if (is_array($categoryIds) && !empty($categoryIds)) {
            $select->where->and->in('melis_ecom_product_category.pcat_cat_id', (int)$categoryIds);
        }
    ...
    public function getProductTitleAndSeoById($productId, $langId)
    {
        ...
        $select->where->NEST->equalTo('melis_ecom_product_text.ptxt_lang_id', (int)$langId)
            ->or->isNull('melis_ecom_product_text.ptxt_lang_id')->UNNEST;

        $select->where->equalTo('melis_ecom_product.prd_id', (int)$productId);
    ...
    public function getProductByVariantId($variantId)
    {
        ...
        $select->where->equalTo('melis_ecom_variant.var_id', (int)$variantId);
    ...
    public function getProductCategoryByProductId($productId, $langId = null)
    {
        ...
        if (!empty($langId)) {
            $select->where->equalTo('melis_ecom_category_trans.catt_lang_id', (int)$langId);
        }
    ...
    public function getProductCategoryPriceByProductId($productId, $categoryIds, $langId, $countryId = null, $fieldsTypeCodes = array('TITLE'), $documents = array('mainImage' => 'DEFAULT'))
    {
        ...
        $select->where->equalTo('melis_ecom_category_trans.catt_lang_id', (int)$langId);
        $select->where->equalTo('melis_ecom_product_text.ptxt_lang_id', (int)$langId);
        ...
        if (!is_null($countryId)) {
            $select->where->equalTo('melis_ecom_price.price_country_id', (int)$countryId);
        }
    ...
    public function getProductsByCategoryId($categoryId, $onlyValid = false, $langId = null, $order = null)
    {
        ...
        $select->where->equalTo('melis_ecom_product_category.pcat_cat_id', (int)$categoryId);
