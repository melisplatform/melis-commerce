...
class MelisEcomPriceTable extends MelisEcomGenericTable
{
    ...
    public function getPricesByVariantId($variantId, $countryId = null, $groupId = 1)
    {
        ...
        $select->join(
            'melis_ecom_currency',
            'melis_ecom_currency.cur_id = melis_ecom_price.price_currency',
            array('*'),
            $select::JOIN_LEFT
        );
        ...
        if ($groupId >= 1)
            $select->where->equalTo('melis_ecom_price.price_group_id', (int)$groupId);
    ...
    public function getPricesByProductId($productId, $countryId = null, $groupId = 1)
    {
        ...
        $select->join(
            'melis_ecom_country',
            'melis_ecom_country.ctry_id = melis_ecom_price.price_country_id',
            array('*'),
            $select::JOIN_LEFT
        );
        $select->join(
            'melis_ecom_currency',
            'melis_ecom_country.ctry_currency_id = melis_ecom_currency.cur_id',
            array('cur_code', 'cur_symbol'),
            $select::JOIN_LEFT
        );
    ...
    public function getItemPrice($type, $itemId, $countryId, $groupId = 1)
    {
        ...
        $select->where->equalTo($typeFk, (int)$itemId)
            ->and->equalTo('price_country_id', (int)$countryId)
            ->and->equalTo('price_group_id', (int)$groupId)
