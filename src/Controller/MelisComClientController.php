...
class MelisComClientController extends MelisAbstractActionController
{
    ...
    public function renderClientPageSaveAction()
    {
        ...
        $canAccess = $rights->isAccessible($xmlRights, 'meliscore_interface', 'meliscommerce_clients_list_save_client_button');
    ...
    public function renderClientAddContactAction()
    {
        ...
        $xmlRights = $melisCoreAuth->getAuthRights();
        $rights = $this->getServiceManager()->get('MelisCoreRights');
        $canAccess = $rights->isAccessible($xmlRights, 'meliscore_interface', 'meliscommerce_client_add_contact');
    ...
    public function addClientContactAction()
    {
        ...
        if ($request->isPost()) {
            ...
            // Checking if the Client Contact form is valid after set the Data from Post
        ...
        if ($request->isPost()) {
            ...
            if ($propertyForm->isValid()) {
                ...
                $tabNav = '<li class="nav-item ' . $addressId . '_address">
                                <a class="nav-link clearfix" data-bs-toggle="tab" id="nav_add_' . $addressId . '" href="#' . $addressId . '_address" aria-expanded="false">
                                    <span>
                                        ' . $addressName . '
                                        <label class="label label-success">' . $translator->translate('tr_meliscommerce_clients_common_label_new') . '</label>
                                    </span>
    ...
    public function saveClientAction()
    {
        ...
        $this->getEventManager()->trigger(
            ...
            $this,
            array_merge($response, array('typeCode' => $logTypeCode, 'itemId' => $clientId))
        );
    ...
    public function deleteAccountAction()
    {
        ...
        $this->getEventManager()->trigger(
            ...
            $this,
            array_merge($response, array('typeCode' => 'ECOM_ACCOUNT_DELETE', 'itemId' => $accountId))
        );
    ...
    public function importAccountsAction()
    {
        ...
        $response = [
            ...
            'typeCode' => 'ECOM_CLIENTS_IMPORT'
        ];

        $this->getEventManager()->trigger('meliscommerce_clients_import_end', $this, $response);
