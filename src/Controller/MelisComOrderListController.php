...
class MelisComOrderListController extends MelisAbstractActionController
{
    ...
    {
        $orderNum = Order::selectRaw('COUNT(ord_id) as count')
                        ->where('ord_status', '!=', -1)
                        ->get()->first();
...
//         $tabId = $this->params()->fromQuery('tabId');
    ...
    public function getOrderListDataAction()
    {
        ...
        if($this->getRequest()->isPost()) {
            ...
            $dataFilter = Order::setToolTipTableHelper($toolTipTable, $this->getToolTipColumns())
                            ->setLangId($langId)
                            ->getToolListOrder(
                                $osta_id,
                                $clientId,
