...
class MelisComReactApiSettingsController extends MelisAbstractActionController
{
    ...
    public function getAction(): HttpResponse
    {
        ...
        try {
            ...
            $recipients = [];
            foreach ($db->query(
                'SELECT sea.sea_id, sea.sea_stock_level_alert, sea.sea_email, sea.sea_user_id, u.usr_firstname, u.usr_lastname
                 FROM melis_ecom_stock_email_alert sea
                 LEFT JOIN melis_core_user u ON u.usr_id = sea.sea_user_id
                 WHERE sea.sea_prd_id = ? ORDER BY sea.sea_id ASC',
                [self::GLOBAL_PRD_ID]
            ) as $r) {
                $r = (array) $r;
                if ($stockLevelAlert === null && $r['sea_stock_level_alert'] !== null) { $stockLevelAlert = (int) $r['sea_stock_level_alert']; }
    ...
    public function saveAction(): HttpResponse
    {
        ...
        try {
            ...
            $existingIds = [];
            foreach ($db->query('SELECT sea_id FROM melis_ecom_stock_email_alert WHERE sea_prd_id = ?', [self::GLOBAL_PRD_ID]) as $r) {
