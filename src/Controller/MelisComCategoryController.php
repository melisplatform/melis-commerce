...
class MelisComCategoryController extends AbstractActionController
{
    ...
    public function renderCategoryFormCountriesAction(){
        ...
        if (!empty($catId)){
            // Getting Category Countries form Category Service
//             $melisComCategoryService = $this->getServiceLocator()->get('MelisComCategoryService');
//             $categoryData = $melisComCategoryService->getCategoryById($catId);
//             $catCountriesData = $categoryData->getCountries();
    ...
    public function saveCategoryAction(){
        ...
        $success = 0;
        $errors  = array();
        $textTitle = $translator->translate('tr_meliscommerce_categories_category_save');
        $textMessage = '';
        $cat_id = 0;
        $catParents = '';
        ...
        if($request->isPost()) {
            ...
            if (empty($errors)){
                ...
                {
                    $cat_id = $results;
                    $textMessage = $translator->translate('tr_meliscommerce_categories_err_category_save_success');
                    $success = 1;
                }
                else
                {
                ...
                $textMessage = $translator->translate('tr_meliscommerce_categories_err_category_save_unable');
            }
        }
    ...
    public function validateCategoryTranslationsAction(){
        ...
        if($request->isPost()) {
            ...
            if (!$hasCategoryName)
            {
                $errors['catt_name'] = array(
                    'label' => $translator->translate('tr_meliscommerce_categories_category_information_form_cat_name'),
                    'NotEmpty' => $translator->translate('tr_meliscommerce_categories_err_category_name_required_atleast_one')
                );
            }
        }
    ...
    public function deleteCategoryAction(){
        ...
        $errors = array();
        $status  = 0;
        $textTitle = $translator->translate('tr_meliscommerce_categories_category_delete');
        $textMessage = '';
        ...
        if($request->isPost()) {
            ...
            if (empty($categoryChildren)){
                ...
                $textMessage = $translator->translate('tr_meliscommerce_categories_category_delete_success');
                $status = 1;
            }else{
                $textMessage = $translator->translate('tr_meliscommerce_categories_err_category_delete_has_children');
            }
        }
    ...
    public function getCategoryProductListAction(){
        ...
        if ($catId){
            ...
            // Tooltip anchor
            $toolTipTextTag = '<a id="row-%s" class="" data-productId="%s" data-hasqtip="1" aria-describedby="qtip-%s">%s</a>';
            $ctr = 0;
            ...
            foreach ($categoryProducts As $val){
                ...
                $toolTipTable->setTable('', 'table-row-'.($ctr+1) . ' ' . 'productTable'.($ctr+1), '');
