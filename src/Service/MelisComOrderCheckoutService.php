            ...
            if (!is_null($clientBasket)) {
                ...
                foreach ($clientBasket as $val) {
                    ...
                    if (!empty($variant)) {
                        ...
                        $prdVarPrice = $melisComPriceService->getItemPrice(
                            $variantId,
                            $clientCountryId,
                            $clientGroupId,
                            'variant',
                            ['method' => 'computeOrderCost', 'basket' => $val]
                        );
        ...
        if ($basketValidity && $addressesValidity && $costsValidity && $orderReferenceCode['success']) {
            ...
            foreach ($clientBasket as $key => $val) {
                ...
                $prdVarPrice = $melisComPriceService->getItemPrice(
                    ...
                    $clientGroupId,
                    'variant',
                    ['method' => 'computeOrderCost', 'basket' => $val, 'skipLogsTranslation' => true]
                );
                ...
                // Sending service end event
                $basketResults = $this->sendEvent(
                    'meliscommerce_service_checkout_step1_prepayment_order_basket_end',
