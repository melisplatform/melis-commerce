...
class MelisComClientService extends MelisComGeneralService
{
	...
	public function getClientList(
		...
		// Get Client Data
		$clientData = $melisEcomClientTable->getClientList(
			$arrayParameters['countryId'],
			$arrayParameters['dateCreationMin'],
			$arrayParameters['dateCreationMax'],
			$arrayParameters['onlyValid'],
			$arrayParameters['start'],
			$arrayParameters['limit'],
			$arrayParameters['order'],
		...
		if ($count) {
			...
			for ($c = 0; $c < $cycles; $c++) {
				...
				$clients = $this->getClientList(
					null,
					$arrayParameters['startDate'],
					$arrayParameters['endDate'],
					$arrayParameters['clientStatus'],
					$start,
					$limit,
					null,
					null
	...
	public function getWidgetClients($identifier, $param = null)
	{
		...
		switch ($arrayParameters['identifier']) {
			case 'curMonth':
				$results = $clientTable->getCurrentMonth()->count();
				break;
			case 'activeInactive':
				$results = $clientTable->getActiveInactive($param)->current();
				break;
			default:
				break;
		}
