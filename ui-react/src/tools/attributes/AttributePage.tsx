...
import {
  fetchAttributes, fetchAllAttributes, fetchAttributeStats, fetchAttributeOptions, fetchAttributeById, saveAttribute, deleteAttribute,
  type AttributeItem, type AttributeStats, type AttributeSortKey, type AttributeOptions, type AttributeDetail, type AttributeTranslation, type LangOption,
} from './api'
...
const listCache = makeCache<{
  items: AttributeItem[]; stats: AttributeStats | null; total: number; cursor: string | null; hasMore: boolean
  search: string; searchInput: string; filterStatus: number | null; filterType: number | null; sortCol: string; sortDir: 'asc' | 'desc'; mode: 'react' | 'old'
}>()
...
function AttributeList({ base }: { base: string }) {
  ...
  const [stats, setStats] = useState<AttributeStats | null>(cached?.stats ?? null)
  ...
  const [filterType, setFilterType] = useState<number | null>(cached?.filterType ?? null)
  const [exportItems, setExportItems] = useState<AttributeItem[]>([])
  ...
  // Scroll infini + tri server-side + keyset (mutualisé).
  const {
    items, setItems, total, loading, hasMore, sentinelRef, sortCol, sortDir, setSortCol, setSortDir, toggleSort, snapshot,
  } = useKeysetList<AttributeItem>({
    fetcher: (a) => fetchAttributes({ ...a, sort: a.sort as AttributeSortKey, search, status: filterStatus, typeId: filterType }),
    deps: [search, filterStatus, filterType, tick],
    defaultSort: 'id',
    defaultDir: 'desc',
  ...
  const cacheRef = useRef({ ...snapshot(), stats, search, searchInput, filterStatus, filterType, mode })
  useEffect(() => { cacheRef.current = { ...snapshot(), stats, search, searchInput, filterStatus, filterType, mode } })
  ...
  async function openExport() {
    setExporting(true)
    try { const all = await fetchAllAttributes({ search, status: filterStatus, typeId: filterType }); setExportItems(all); setShowExport(true) }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: 24, boxSizing: 'border-box', ...(mode === 'old' ? { height: '100%', overflow: 'hidden' } : {}) }}>
      ...
      <div style={{ display: mode === 'react' ? 'flex' : 'none', flexDirection: 'column', gap: 20, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
          ...
          <select style={{ ...inputCss, height: 36, width: narrow ? '100%' : 'auto', minWidth: 160, flexBasis: narrow ? '100%' : undefined }}
            value={filterType ?? ''} onChange={(e) => setFilterType(e.target.value === '' ? null : Number(e.target.value))}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: narrow ? undefined : 700 }}>
            <thead style={{ background: 'var(--color-muted,rgba(0,0,0,.03))' }}>
              <tr>
                ...
                {visible.map((c) => {
                  const sortable = SORTABLE.has(c.id as AttributeSortKey)
                  return (
      ...
      {showExport && (
        <ExportModal cols={cols} items={exportItems} getCell={(a, id) => getCellExport(a, id)}
