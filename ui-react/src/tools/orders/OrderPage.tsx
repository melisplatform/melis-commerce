...
import {
  fetchOrders, fetchAllOrders, fetchOrderStats, fetchOrderStatuses, fetchOrderById,
  createOrder, saveOrderInfo,
  fetchOrderAttachments, uploadOrderAttachment, deleteOrderAttachment,
  type OrderItem, type OrderStats, type OrderStatus, type OrderDetail, type OrderAttachment, type OrderSortKey,
} from './api'
...
import { CartIcon, ShoppingCartKpiIcon, PackageIcon, CalendarIcon, PlusIcon, RefreshIcon, PencilIcon, ResetIcon, TrashIcon, SortIcon, Spinner } from '../../shared/icons'
...
const listCache = makeCache<{
  items: OrderItem[]; stats: OrderStats | null; total: number; cursor: string | null; hasMore: boolean
...
function OrderList({ base }: { base: string }) {
  ...
  const [stats, setStats] = useState<OrderStats | null>(cached?.stats ?? null)
  ...
  const [dateStart, setDateStart] = useState(cached?.dateStart ?? '')
  const [dateEnd, setDateEnd] = useState(cached?.dateEnd ?? '')
  const [tick, setTick] = useState(0)
  const [cols, setCols] = useState<ColDef[]>(cols$.load)
  const [showCols, setShowCols] = useState(false)
  const [showExport, setShowExport] = useState(false)
  const [exportItems, setExportItems] = useState<OrderItem[]>([])
  ...
  } = useKeysetList<OrderItem>({
    fetcher: (a) => fetchOrders({ ...a, sort: a.sort as OrderSortKey, search, status: filterStatus, dateStart, dateEnd }),
    deps: [search, filterStatus, dateStart, dateEnd, tick],
    defaultSort: 'id',
    defaultDir: 'desc',
  ...
  const cacheRef = useRef({ ...snapshot(), stats, search, searchInput, filterStatus, dateStart, dateEnd, mode })
  useEffect(() => { cacheRef.current = { ...snapshot(), stats, search, searchInput, filterStatus, dateStart, dateEnd, mode } })
  ...
  async function openExport() {
    setExporting(true)
    try { const all = await fetchAllOrders({ search, status: filterStatus, dateStart, dateEnd }); setExportItems(all); setShowExport(true) }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: 24, boxSizing: 'border-box', ...(mode === 'old' ? { height: '100%', overflow: 'hidden' } : {}) }}>
      ...
      <div style={{ display: mode === 'react' ? 'flex' : 'none', flexDirection: 'column', gap: 20, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
          ...
          <select style={{ ...inputCss, height: 36, width: narrow ? '100%' : 'auto', minWidth: 160, flexBasis: narrow ? '100%' : undefined }}
            value={filterStatus ?? ''} onChange={(e) => setFilterStatus(e.target.value ? Number(e.target.value) : null)}>
          ...
          <div style={{ flexBasis: narrow ? '100%' : undefined }}>
            <DateRangeFilter from={dateStart} to={dateEnd}
              onChange={(f, to) => { setDateStart(f); setDateEnd(to) }} t={t} />
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: narrow ? undefined : 700 }}>
            <thead style={{ background: 'var(--color-muted,rgba(0,0,0,.03))' }}>
              <tr>
                ...
                {visible.map((c) => {
                  const sortable = SORTABLE.has(c.id as OrderSortKey)
                  return (
      ...
      {showExport && (
        <ExportModal cols={cols} items={exportItems} getCell={(o, id) => getCellExport(o, id)}
