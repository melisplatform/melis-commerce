<?php 
return array(
    // General Messages
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_PRODUCT_NOT_EXISTING' => 'Variant not existing',
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_VARIANT_NOT_EXISTING' => 'Product not existing',
    
    // Basket
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_BASKET_QUANTITY' => 'Quantity higher than the product stock',
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_BASKET_VARIANT_NOT_ACTIVE' => 'Product not active',
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_BASKET_VARIANT_NO_STOCKS' => 'Product no stocks',
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_BASKET_VARIANT_QUANTITY_NOT_SET' => 'Product quantity not set',
    
    // Price
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_PRODUCT_PRICE_IS_ZERO' => 'Variant total Amount is 0 (Zero)',
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_PRODUCT_PRICE_NOT_SET' => 'Variant price is not set',
    
    //Addresses
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_ADDRESS_NOT_ACCEPTED' => 'Address not accepted',
    
    'tr_MELIS_COMMERCE_CHECKOUT_ERROR_SHIPMENT_CANT_COMPUTE_PRODUCT' => 'Shipment price can\'t be computed for product',
    
);