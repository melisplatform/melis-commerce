	...
	function setCustomNodeText(targetId) {
		$("#"+targetId+".jstree li.jstree-node .jstree-anchor").each(function(){
    		var nodeTargetAnchor 	= $(this),
				textlang 			= nodeTargetAnchor.data('textlang'),
				spanHtml 			= '<span title="' + translations.tr_meliscommerce_categories_list_tree_view_product_num + '"></span>';
	...
	function categoryTreeEvents(targetId) {
		...
		$("#catPrdSliderPluginSearch").on("keyup", function(e) { 
			var $this 			= $(this),
				...
				searchResult 	= categoryTree.jstree('search', searchString);
	...
	var $body = $("body");
		...
		$body.on("click", ".categoryProductsRemove", function(){
			var $this = $(this),
				...
				deleteTitle = translations.tr_meliscommerce_categories_category_product_remove,
				deleteMessage = translations.tr_meliscommerce_categories_category_product_remove_confirm_msg;
			...
			.on('after_open.jstree', function (e, data) {
				$.each(data.node.children_d, function(k, v) {
					var textlang 	= $('#'+v+'_anchor').data('textlang'),
						...
						spanHtml 	= '<span title="' + translations.tr_meliscommerce_categories_list_tree_view_product_num + '">('+ products +')</span>',
							...
							"action" : function (obj) {
								...
								var dataString = new Array(),
									...
									var deleteTitle 	= translations.tr_meliscommerce_categories_category_delete,
										deleteMessage 	= translations.tr_meliscommerce_categories_category_delete_confirm_msg;
...
$(function() {
    var $body = $("body");
        ...
        $body.on("click", function (e) {
            var $comModalCon    = $('#id_meliscommerce_client_modal_address_form_container'),
                $comModalAddCon = $('#id_meliscommerce_client_modal_contact_address_form_container');
                        ...
                        var $clientContFirst    = $("."+tabClass+"_client_contact:first"),
                            ...
                            if ( $clientContFirst.hasClass("active") ) {
                                if ( $clientContFirst.next().find("a").length ) {
                                    // Activate next tab after removing current active tab
                                    $clientContFirst.next().find("a").tab("show");
                                }
                                else if ( $clientContFirst.prev().find("a") ) {
                                    // Activate previous tab after removing current active tab
                                    $clientContFirst.prev().find("a").tab("show");
                                }
                            }
        ...
        $body.on("click", ".clientOrderView", function(){
            var $this           = $(this),
                ...
                orderRef        = $this.closest('tr').find("td:nth-child(2)").text(),
        ...
        $body.on("click", "#exportClients", function() {
            var button      = $(this),
                formValues  = button.closest('#id_meliscommerce_client_list_content_export_form').find('form').serializeArray(),
...
window.initClientContactAddressForm = function(){
    var tabId           = $("#saveClientContactAddress").data("tabid"),
        ...
        $addFormModal.find("#cadd_civility").val( $("#"+tabId+"_contact_form").find("#cper_civility").val() );
    ...
    var $body           = $("body"),
        ...
        $body.on("click", ".saveOrder", function(){
            var $this       = $(this),
                ...
                len,
                ctr         = 0,
                statusId    = $this.closest('.container-level-a').find('.selectedStatus').data('statusid'),
                reference   = $this.closest('.container-level-a').find('input[name=ord_reference]').val(),
        ...
        $body.on("click", "#saveOrderStatus", function() {
            var $this       = $(this),
                dataString  = $this.closest('#id_meliscommerce_order_list_content_status_form').find('form').serializeArray(),
                url         = 'melis/MelisCommerce/MelisComOrderList/saveOrderStatus',
                status      = $('#id_meliscommerce_order_list_content_status_form select[name="ord_status"]').val(),
                reference   = translations.tr_meliscommerce_orders_Order + ' ' +$this.data('reference');
        ...
        $body.on("click", "#saveOrderShipping", function() {
            var $this           = $(this),
                forms           = $this.closest('#id_meliscommerce_order_modal_content_shipping_form').find('form'),
                url             = 'melis/MelisCommerce/MelisComOrder/saveOrder',
        ...
        $body.on('apply.daterangepicker', ".dt_orderdatepicker", function(ev, picker) {
            // reload table
            var $this   = $(this),
                tableId = $this.parents().eq(5).find('table').attr('id');
        ...
        $body.on("click", ".variantInfo", function() {
            var $this        = $(this),
                ...
                productId    = $this.closest('tr').data('productid'),
                productName  = $this.closest('tr').find('td:nth-child(3)').text(),
                prodTabId    = productId+"_id_meliscommerce_products_page",
                navTabsGroup = "id_meliscommerce_product_list_container";
        ...
        $body.on("click", "#exportOrders", function() {
            var button      = $(this),
                formValues  = button.closest('#id_meliscommerce_order_list_content_export_form').find('form').serializeArray(),
        ...
        $body.on("click", "#saveOrderStatusForm", function() {
            var saveButton  = $(this),
                ...
                statusDom   = $('#id_meliscommerce_order_status_form_container .make-switch div').hasClass('switch-on'),
    ...
    var $body = $("body");
        ...
        $body.on("click", ".orderPaymentCouponLink", function() {
            var $this       = $(this),
                ...
                alreadyOpen = $("body #melis-id-nav-bar-tabs li a.tab-element[data-id='id_meliscommerce_coupon_list_page']");
        ...
        $body.on("click", ".couponClientListRefresh", function() {
            var $this    = $(this),
                parentId = $this.parents().eq(5).attr('id'),
                meliskey = $this.parents().eq(5).data('meliskey'),
        ...
        $body.on("click", ".removeCouponFromClient", function(){ 
            var $this       = $(this),
                ...
                dataString  = [],
                ccliId      = $this.closest('td').prev().find('a').data('ccli_id');
        ...
        $body.on("click", ".couponInfo", function() {
            var $this      = $(this),
                ...
                couponCode = $this.closest('tr').find("td:nth-child(3)").text(),
                tabName    = couponId;
...
window.initCouponOrder = function(data, tblSettings) {
    ...
    var icon = '<i class="glyphicon glyphicon-calendar fa fa-calendar"></i> ';
        ...
        if ( tblSettings.iDraw > 1 ) {
            dateSelectionContent = translations.tr_meliscore_datepicker_select_date  + icon + "<span class='sdate'>" + dStartDate + ' - ' + dEndDate + '</span> <strong class="caret"></strong>';
			...
			var $this 		= $(this),
				...
				if ( row.child.isShown() ) {
					...
					var zoneId 				= productId+"_id_meliscommerce_order_checkout_product_variant_list",
						...
						// Extra row Container
						variantContainer 	= '<div class="checkout-product-variant-list" id="'+zoneId+'" melisKey="'+melisKey+'"></div>';
...
$(function() {
    var $body = $("body");
        ...
        $body.on("click", ".melisComDuplicateVariant", function(){
            var btn         = $(this),
                variantId   = btn.parents("tr").attr('id');
        ...
        $body.on("click", ".melisComDuplicateProduct", function(){
            var btn         = $(this),
                productId   = btn.parents("tr").attr('id');
        ...
        $body.on("click", "#melisComStartDuplicateVariant", function(){
            var btn = $(this),
                ...
                $("#id_meliscommerce_variant_duplication form").each(function() {
                    var $this       = $(this),
                        var_id      = $this.find("input[name='var_id']").val(),
                        tempData    = $this.serializeArray();
        ...
        $body.on("click", ".main-variant-radio i", function() {
            var $this               = $(this),
                $variantRadioIcon   = $(".main-variant-radio i");

                $this.addClass("fa-dot-circle-o");
                $this.addClass("text-success");

                $variantRadioIcon .not(this).addClass("fa-circle-o");
                $variantRadioIcon .not(this).removeClass("fa-dot-circle-o");
                $variantRadioIcon .not(this).removeClass("text-success");
        })
...
function melisSKUKoNotification(title, message, errors, closeByButtonOnly){
    ...
    var errorTexts = '<h3>'+ melisHelper.melisTranslator(title) +'</h3>';
        ...
        $.each( errors, function( key, error ) {
            if(key !== 'label'){
                errorTexts += '<p class="modal-error-cont"><strong>'+ (( errors[key]['label'] == undefined ) ? ((errors['label']== undefined) ? key : errors['label'] ) : errors[key]['label'] )+ ': </strong>  ';
                // catch error level of object
                try {
                    $.each( error, function( key, value ) {
                        if(key !== 'label' && key !== 'form'){
