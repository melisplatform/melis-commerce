$(function () {
			...
			.done(function (data) {
				...
				if (data.success) {
					...
					melisCoreTool.hideModal(
						"id_meliscommerce_client_modal_contact_form_container"
					);
	...
	$body.on("click", ".cper_type", function () {
		...
		if ($(this).val() == "company") {
			...
			form
				...
				.find("label")
				.html(
					translations.tr_meliscommerce_contact_common_company +
						" " +
						"<sup>*</sup>"
				);
		} else {
			...
			form
				...
				.find("label")
				.html(
					translations.tr_meliscommerce_contact_firstname + " " + "<sup>*</sup>"
				);
		}
	});
	...
	$body.on(
		"click",
		"#melisCommerceClientContactFormModal .cper_type",
		function () {
			...
			if ($(this).val() == "company") {
				...
				form
					...
					.find("label")
					.html(
						translations.tr_meliscommerce_contact_firstname +
							" " +
							"<sup>*</sup>"
					);
			}
		...
		$.ajax({
			...
			.done(function (data) {
				...
				if (data.success) {
					...
					melisHelper.zoneReload(
						contactId + "_id_meliscommerce_contact_page_content_tab_address",
						"meliscommerce_contact_page_content_tab_address",
						{ contactId: contactId, reload: true }
					);
				} else {
					melisHelper.melisKoNotification(
						data.textTitle,
	...
	$body.on("click", ".updateDefaultAccount", function () {
		...
		$.ajax({
			...
			if (data.success) {
				$("#" + contactId + "_contactAssocAccountList")
					.DataTable()
					.ajax.reload();
				melisHelper.melisOkNotification(data.textTitle, data.textMessage);
				...
				if ($("#id_meliscommerce_clients_list_page").length > 0) {
					...
					// }
					$(
						"li[data-tool-id='id_meliscommerce_clients_list_page'] .nav-group-dropdown li"
					).each(function () {
						var tableAccountId = $(this)
							.data("tool-id")
							.replace("_id_meliscommerce_client_page", "");
						melisHelper.zoneReload(
	...
	$body.on("click", ".saveContactPage", function () {
		...
		var dataString = $(
			"#" +
				contactId +
				"_id_meliscommerce_contact_page_content_tab_information form"
		).serializeArray();
		...
		$(
			"#" +
				contactId +
				"_id_meliscommerce_contact_page_content_tab_address form"
		).each(function () {
			var $this = $(this),
			...
			$.each(clientContactAddressForm, function () {
				dataString.push({
					name: "contactAddress[" + contactAddressId + "][" + this.name + "]",
					value: this.value,
				});
			});
		});
	...
	$body.on("click", "#exportContacts", function (e) {
		...
		$.ajax({
			...
			success: function (response, status, request) {
				...
				melisCoreTool.hideModal(
					"id_meliscommerce_contact_list_export_contacts_form_container"
				);
			},
			...
			.done(function (data) {
				if (data.success) {
					if (type == "import") {
						importContacts(
							formData,
							"/melis/MelisCommerce/MelisComContact/importContacts",
							type
						);
					} else {
						importContacts(
							formData,
							"/melis/MelisCommerce/MelisComContact/testImportContacts",
							type
						);
					}
				} else {
				...
				setTimeout(function () {
					...
					if (data.success) {
						...
						if (type == "import") {
							...
							melisCoreTool.hideModal(
								"id_meliscommerce_contact_list_import_contacts_form_container"
							);
	...
	$body.on("click", ".link", function () {
		...
		$(href).find(".refreshAsocc").trigger("click");
	});
	...
	$body.on("click", ".parents", function () {
		...
		$(href).find(".accountContactListRefresh").trigger("click");
	});
});
...
window.setContactId = function (d) {
	d.contactId =
		accountToolSelectedContact != ""
			? accountToolSelectedContact
			: activeTabId.replace("_id_meliscommerce_contact_page", "");
};
...
window.initAccountAutoSuggest = function ($element) {
	let options = {
		url: function (searchPhrase) {
			...
			return (
				"/melis/MelisCommerce/MelisComContact/fetchAllAccount?phrase=" +
				searchPhrase +
				"&contactId=" +
				contactId
			);
		...
		list: {
			...
			onChooseEvent: function () {
				...
				$("#" + activeTabId + " button.contactAccountLink").prop(
					"disabled",
					false
				);
			},
...
window.contactAssociatedAccountCallback = function () {
	...
	$("#" + contactId + "_contactAssocAccountList tbody tr").each(function () {
		...
		if (isDefault == 1) {
			...
			$this
				.find("button.updateDefaultAccount")
